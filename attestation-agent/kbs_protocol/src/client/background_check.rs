...
impl KbsClient<BoxedEvidenceProvider> {
    ...
    async fn rcar_handshake(&mut self) -> Result<()> {
        if self._tee.is_none() {
            // we can unwrap() here because the evidence_provider is asserted when
            // the rcar_handshake is called

            let tee = self.provider.get_tee_type().await?;
            self._tee = Some(tee);
        }

        assert!(self._tee.is_some());
        let auth_endpoint = format!("{}/{KBS_PREFIX}/auth", self.kbs_host_url);
        let request = Request {
            version: String::from(KBS_PROTOCOL_VERSION),
            tee: self._tee.clone().unwrap(),
