...
if ! kill -0 "$keyprovider_pid"; then
	bold_echo "coco_keyprovider failed to start"
	exit 1
fi

popd

# bold_echo "Start local docker registry..."
...
CARGO_TARGET_X86_64_UNKNOWN_LINUX_GNU_RUNNER='sudo -E' cargo test \
	--features encryption-ring,getresource,oci-distribution/rustls-tls-native-roots \
